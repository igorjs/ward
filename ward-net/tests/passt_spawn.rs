...
mod passt_spawn_tests {
    use ward_net::passt::{spawn_for_sandbox, PasstHandle};
    ...
    async fn given_passt_available_when_spawn_then_returns_valid_fd_and_child() {
        ...
        assert!(flags >= 0, "guest_fd {} is not a valid open fd", handle.guest_fd);
    ...
    async fn given_passt_spawned_when_kill_twice_then_idempotent() {
        ...
        handle.kill().await.expect("second kill must not panic or error");
    }
