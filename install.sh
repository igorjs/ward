...
# ---------------------------------------------------------------------------
# SLSA build-provenance verification (SEC-012)
# ---------------------------------------------------------------------------
#
# Sigstore-backed attestation generated by ward's release.yml. Proves
# the tarball was built by igorjs/ward's release workflow at a known
# SHA, by GitHub's official runners, with no post-build tampering.
# Defeats compromised-release-token attacks that the same-Release
# SHA-256 sidecar above cannot — a leaked release-publish token would
# let an attacker upload a matching .sha256 alongside a malicious
# tarball, but they cannot forge a sigstore attestation chained to a
# Rekor transparency log entry.
#
# Requires `gh` (https://cli.github.com). If unavailable, we WARN and
# continue with SHA-256-only verification; users who want the strongest
# integrity check should install gh and re-run.
...
if command -v gh > /dev/null 2>&1; then
  ...
  if gh attestation verify \
       ...
       --owner "${OWNER}" \
       > /dev/null 2>&1; then
    ok "Build provenance verified (signed by ${WARD_REPO}/release.yml)"
  else
    err "SLSA attestation verification FAILED — refusing to install"
    err "Run for the full output:"
    err "  gh attestation verify ${TMP_DIR}/${ARCHIVE} --owner ${OWNER}"
    exit 4
  fi
else
  warn "gh CLI not found — skipping SLSA attestation verification."
  warn "SHA-256 protects against transport tampering only; for full"
  warn "supply-chain integrity, install gh (https://cli.github.com)"
